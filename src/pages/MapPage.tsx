...
export function MapPage() {
  ...
  return <PageShell><main className="cycle-page map-page"><header className="page-heading"><div><p className="kicker">Карта сообщества</p><h1>Дорога вокруг тебя</h1><p>Маршруты, заезды, опасности и объявления собраны в одном месте.</p></div>{tab === 'routes' && <Link className="signal-button" href="/routes/new">Нарисовать маршрут</Link>}</header><nav className="section-tabs" aria-label="Разделы карты">{tabs.map(({ id, label, icon: Icon }) => <button className={tab === id ? 'active' : ''} key={id} onClick={() => setTab(id)}><Icon size={17} aria-hidden="true" />{label}</button>)}</nav>{error && <div className="inline-error" role="alert">{error}<button onClick={() => void refresh()}>Повторить</button></div>}{!ready ? <p className="loading-copy">Собираем карту…</p> : <>{tab === 'routes' && (routes.length ? <section className="route-grid">{routes.map((route) => <RoutePreview route={route} key={route.id} />)}</section> : <section className="empty-panel"><h2>Маршрутов ещё нет</h2><p>Нарисуй первый трек, которому доверяешь.</p><Link className="signal-button" href="/routes/new">Создать маршрут</Link></section>)}{tab === 'rides' && <section className="explore-list">{groupRides.length ? groupRides.map((ride) => <article key={ride.id}><Bike size={22} aria-hidden="true" /><div><strong>{ride.title}</strong><span>{formatDate(ride.scheduled_at)}{ride.max_participants ? ` · до ${ride.max_participants} райдеров` : ''}</span><p>{ride.description || 'Описание заезда появится у организатора.'}</p></div></article>) : <section className="empty-panel"><h2>Ближайших заездов пока нет</h2><p>Как только райдеры назначат совместную поездку, она появится здесь.</p></section>}</section>}{tab === 'hazards' && <section className="explore-list">{hazards.length ? hazards.map((hazard) => <article key={hazard.id}><AlertTriangle size={22} aria-hidden="true" /><div><strong>{hazardLabels[hazard.hazard_type]}</strong><span>Подтверждений: {hazard.upvotes}</span><p>{hazard.description || 'Райдер отметил этот участок на карте.'}</p></div></article>) : <section className="empty-panel"><h2>Активных опасностей нет</h2><p>На карте пока нет отмеченных проблемных участков.</p></section>}</section>}{tab === 'marketplace' && <section className="marketplace-grid">{marketplace.length ? marketplace.map((listing) => <article key={listing.id}>{listing.photos[0] && <img src={listing.photos[0]} alt="" />}<div><p>{listing.category} · {listing.condition}</p><h2>{listing.title}</h2><strong>{Number(listing.price).toLocaleString('ru-RU')} ₸</strong><span>{listing.city || 'Город не указан'}</span><small>{listing.description}</small></div></article>) : <section className="empty-panel"><h2>Объявлений пока нет</h2><p>Когда кто-то выставит велосипед или запчасть, она появится здесь.</p></section>}</section>}</>}</main></PageShell>;
}
