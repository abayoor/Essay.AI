...
import { ArrowLeft, Paperclip, Send, Share2 } from 'lucide-react';
...
export function ConversationPage() {
  ...
  const appendMessage = useCallback((message: DirectMessage) => setMessages((current) => current.some((item) => item.id === message.id) ? current : [...current, message]), []);
  useEffect(() => { if (!conversationId || !session) return; return subscribeToConversation(conversationId, appendMessage); }, [appendMessage, conversationId, session]);
  useEffect(() => { endRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages]);
  const otherRider = participants.find((participant) => participant.id !== session?.user.id);

  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(''); try { const message = await sendTextMessage(conversationId, text); setText(''); appendMessage(message); } catch { setError('Не удалось отправить сообщение.'); } finally { setBusy(false); } }
  async function attachFile(file: File | null) { if (!file) return; setBusy(true); setError(''); try { appendMessage(await uploadMessageFile(conversationId, file)); } catch { setError('Не удалось загрузить файл.'); } finally { setBusy(false); } }
  async function showPosts() { setSharing(true); try { setSharePosts(await loadPosts()); } catch { setError('Не удалось загрузить посты.'); } }
  async function sharePost(postId: string) { setBusy(true); try { appendMessage(await sendSharedPost(conversationId, postId)); setSharing(false); } catch { setError('Не удалось прикрепить пост.'); } finally { setBusy(false); } }

  return <PageShell><main className="cycle-page conversation-page">{conversationLoading ? <BikeLoader label="Загружаем диалог…" /> : <section className="conversation-surface">{otherRider && <header className="conversation-header"><Link href="/messages" className="conversation-back-button" aria-label="Выйти из диалога"><ArrowLeft size={20} aria-hidden="true" /><span>Диалоги</span></Link><Link href={`/u/${otherRider.username}`} className="conversation-person"><Avatar profile={otherRider} className="conversation-avatar" /><span><strong>{otherRider.full_name || otherRider.username}</strong><small>@{otherRider.username}</small></span></Link></header>}{error && <p className="inline-error">{error}</p>}<section className="message-thread">{messages.map((message) => <motion.article initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 310, damping: 26 }} className={message.sender_id === session?.user.id ? 'message-bubble mine' : 'message-bubble'} key={message.id}>{message.content_type === 'text' && <p>{message.text_content}</p>}{['image', 'video', 'file'].includes(message.content_type) && <MessageAttachment message={message} />}{message.content_type === 'shared_post' && <SharedPostMessage postId={message.shared_post_id} />}<time>{formatTime(message.created_at)}</time></motion.article>)}<div ref={endRef} /></section>{sharing && <section className="share-post-picker"><div><strong>Прикрепить пост</strong><button type="button" onClick={() => setSharing(false)}>Закрыть</button></div>{sharePosts.slice(0, 12).map((post) => <button type="button" key={post.id} disabled={busy} onClick={() => void sharePost(post.id)}><span>{post.author.username}</span>{post.caption || 'Пост с медиа'}</button>)}</section>}<form className="message-composer" onSubmit={(event) => void submit(event)}><label className="attachment-button" title="Прикрепить файл"><Paperclip size={20} aria-hidden="true" /><input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,application/pdf" onChange={(event) => void attachFile(event.target.files?.[0] ?? null)} /></label><button type="button" className="attachment-button" title="Прикрепить пост" onClick={() => void showPosts()}><Share2 size={19} aria-hidden="true" /></button><EmojiPicker className="message-emoji-picker" onPick={(emoji) => setText((current) => `${current}${emoji}`)} /><input value={text} onChange={(event) => setText(event.target.value)} placeholder="Напиши сообщение…" maxLength={4000} /><button className="message-send-button" disabled={busy || !text.trim()} aria-label="Отправить сообщение"><Send size={20} aria-hidden="true" /></button></form></section>}</main></PageShell>;
}
