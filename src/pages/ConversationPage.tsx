...
function SharedPostMessage({ postId }: { postId: string | null }) {
  ...
  return <Link className="shared-post-message" href={`/feed#post-${postId}`}><span>↗</span><div><strong>Публикация из ленты</strong><small>Открыть прикреплённый пост</small></div></Link>;
}
...
export function ConversationPage() {
  ...
  return <PageShell><main className="cycle-page conversation-page">{otherRider && <header className="conversation-header"><Link href="/messages" className="back-link">← Диалоги</Link><Link href={`/u/${otherRider.username}`} className="conversation-person"><Avatar profile={otherRider} className="conversation-avatar" /><strong>{otherRider.full_name || otherRider.username}</strong></Link></header>}{error && <p className="inline-error">{error}</p>}<section className="message-thread">{messages.map((message) => <article className={message.sender_id === session?.user.id ? 'message-bubble mine' : 'message-bubble'} key={message.id}>{message.content_type === 'text' && <p>{message.text_content}</p>}{['image', 'video', 'file'].includes(message.content_type) && <MessageAttachment message={message} />}{message.content_type === 'shared_post' && <SharedPostMessage postId={message.shared_post_id} />}<time>{formatTime(message.created_at)}</time></article>)}<div ref={endRef} /></section>{sharing && <section className="share-post-picker"><div><strong>Прикрепить пост</strong><button onClick={() => setSharing(false)}>Закрыть</button></div>{sharePosts.slice(0, 12).map((post) => <button key={post.id} disabled={busy} onClick={() => void sharePost(post.id)}><span>{post.author.username}</span>{post.caption || 'Пост с медиа'}</button>)}</section>}<form className="message-composer" onSubmit={(event) => void submit(event)}><label className="attachment-button" title="Прикрепить файл">＋<input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,application/pdf" onChange={(event) => void attachFile(event.target.files?.[0] ?? null)} /></label><button type="button" className="attachment-button" title="Прикрепить пост" onClick={() => void showPosts()}>↗</button><input value={text} onChange={(event) => setText(event.target.value)} placeholder="Напиши сообщение…" maxLength={4000} /><button className="signal-button" disabled={busy || !text.trim()}>Отправить</button></form></main></PageShell>;
}
