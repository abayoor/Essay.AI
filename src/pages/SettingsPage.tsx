...
import type { Bike, Locale, RiderProfile, SocialPost, ThemePreference } from '../lib/cyclingModels';
...
export function SettingsPage() {
  ...
  useEffect(() => { if (!loading && !session) navigate('/auth/sign-in'); if (session) void refresh().catch(() => setMessage('Не удалось загрузить настройки.')); }, [loading, navigate, refresh, session]);

  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const username = profile.username.trim().toLowerCase();
    if (!/^[a-z0-9][a-z0-9_-]{2,47}$/.test(username)) {
      setMessage('Никнейм: 3–48 символов, только латиница, цифры, _ и - .');
      return;
    }
    ...
    try {
      ...
      setMessage(error instanceof Error && error.message.includes('duplicate') ? 'Этот никнейм уже занят.' : 'Не удалось сохранить настройки.');
  ...
  async function changeAvatar(file: File | null) {
    if (!file) return;
    setBusy(true); setMessage('');
    try { const avatarUrl = await uploadAvatar(file); setProfile((current) => ({ ...current, avatar_url: avatarUrl })); setMessage('Аватар загружен. Нажми «Сохранить настройки».'); }
  ...
  async function removePost(id: string) { if (!window.confirm('Удалить безвозвратно?')) return; try { await deletePost(id); await refresh(); } catch { setMessage('Не удалось удалить пост.'); } }
  async function removeBike(id: string) { if (!window.confirm('Удалить безвозвратно? Заезды останутся в журнале без привязки к велосипеду.')) return; try { await deleteBike(id); await refresh(); } catch { setMessage('Не удалось удалить велосипед.'); } }
  async function removeListing(id: string) { if (!window.confirm('Удалить безвозвратно?')) return; try { await deleteMarketplaceListing(id); await refresh(); } catch { setMessage('Не удалось удалить объявление.'); } }
  async function connectStrava() { setBusy(true); setMessage(''); try { await startStravaConnection(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Не удалось открыть Strava.'); setBusy(false); } }
