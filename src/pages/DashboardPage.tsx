...
import { ArrowRight, BadgeCheck, BrainCircuit, ShieldCheck, Trophy } from 'lucide-react';
...
export function DashboardPage() {
  ...
  return <PageShell><main className="cycle-page dashboard-page">
    ...
    {session && ready && stats && <section className="dashboard-coach-teaser"><span><BrainCircuit size={28} aria-hidden="true" /></span><div><p className="kicker">{t('Тренировочный совет', 'Жаттығу кеңесі', 'Training guidance')}</p><h2>{t('Что делать на следующей тренировке?', 'Келесі жаттығуда не істеу керек?', 'What should your next workout be?')}</h2><p>{t('Тренер оценит последние поездки, нагрузку и самочувствие, а затем предложит безопасный план.', 'Жаттықтырушы соңғы сапарларды, жүктемені және көңіл-күйіңді бағалап, қауіпсіз жоспар ұсынады.', 'The coach reviews recent rides, training load and how you feel, then suggests a safe plan.')}</p></div><Link className="signal-button" href="/coach">{t('Открыть тренера', 'Жаттықтырушыны ашу', 'Open coach')}</Link></section>}
