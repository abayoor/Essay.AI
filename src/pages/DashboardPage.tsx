...
export function DashboardPage() {
  ...
  return <PageShell><main className="cycle-page dashboard-page">
    ...
    {session && ready && stats && <section className="dashboard-coach-teaser"><span><BrainCircuit size={28} aria-hidden="true" /></span><div><p className="kicker">{t('ИИ-тренер Pro', 'Pro AI жаттықтырушысы', 'Pro AI coach')}</p><h2>{t('Что делать на следующей тренировке?', 'Келесі жаттығуда не істеу керек?', 'What should your next workout be?')}</h2><p>{t('Pro анализирует последние поездки, нагрузку и самочувствие, а затем предлагает безопасный план.', 'Pro соңғы сапарларды, жүктемені және көңіл-күйді талдап, қауіпсіз жоспар ұсынады.', 'Pro reviews recent rides, training load and how you feel, then suggests a safe plan.')}</p></div><Link className="signal-button" href="/pro#ai-coach">{t('Открыть в Pro', 'Pro ішінде ашу', 'Open in Pro')}</Link></section>}
