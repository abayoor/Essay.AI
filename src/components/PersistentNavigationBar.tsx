...
export function PersistentNavigationBar() {
  ...
  return <aside
    className="persistent-navigation-bar"
    aria-label={text('Сохранённый маршрут', 'Сақталған бағыт', 'Saved route')}
    aria-live="polite"
  >
    ...
    <button
      type="button"
      className="persistent-navigation-resume"
      aria-label={`${text('Продолжить', 'Жалғастыру', 'Resume')}: ${navigation.destinationName}`}
      onClick={() => navigate('/map')}
    >
      <Navigation size={16} aria-hidden="true" />
      {text('Продолжить', 'Жалғастыру', 'Resume')}
    </button>
