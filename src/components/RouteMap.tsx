...
export function RouteMap({ points, className = '' }: { points: RoutePoint[]; className?: string }) {
  ...
  return (
    <MapContainer center={center} zoom={positions.length ? 12 : 11} scrollWheelZoom={false} className={`route-map ${className}`}>
      ...
      {positions.map((position, index) => <CircleMarker center={position} key={`${position[0]}-${position[1]}`} radius={index === 0 ? 7 : 4} pathOptions={{ color: '#fff', fillColor: index === 0 ? '#3e6f5c' : '#ff5a1f', fillOpacity: 1, weight: 2 }} />)}
    </MapContainer>
  );
}
