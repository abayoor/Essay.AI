...
export function RouteMap({ points, className = '' }: { points: RoutePoint[]; className?: string }) {
  ...
  return (
    <MapContainer center={center} zoom={positions.length ? 12 : 11} scrollWheelZoom={false} className={`route-map ${className}`}>
      ...
      {segments.map((segment, index) => segment.length > 1 && <Polyline key={`route-segment-${index}`} positions={segment} pathOptions={{ color: '#26bfa2', weight: 5, lineCap: 'round', lineJoin: 'round' }} />)}
      {positions[0] && <CircleMarker center={positions[0]} radius={7} pathOptions={{ color: '#fff', fillColor: '#3e6f5c', fillOpacity: 1, weight: 2 }} />}
      {positions.length > 1 && <CircleMarker center={positions[positions.length - 1]} radius={6} pathOptions={{ color: '#fff', fillColor: '#26bfa2', fillOpacity: 1, weight: 2 }} />}
    </MapContainer>
  );
}
