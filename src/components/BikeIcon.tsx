...
export function BikeIcon({ type }: { type: BikeType }) {
  ...
  return <svg className="bike-icon" viewBox="0 0 96 56" aria-label={`${text('Велосипед', 'Велосипед', 'Bike')}: ${type}`}><circle cx="20" cy="39" r="13" /><circle cx="80" cy="39" r="13" /><path d={frame} /><path d="M58 12h11M65 12l5 7" /></svg>;
}
