...
import { Bike, BrainCircuit, CircleUserRound, Compass, LogOut, Map, MessageCircle, Moon, Newspaper, Plus, Settings, Sun, Trophy, Wrench } from 'lucide-react';
...
export function PageShell({ children }: { children: ReactNode }) {
  ...
  return <div className="cycle-shell">
    <header className="cycle-header">
      ...
      {session ? <div className="header-session-actions"><nav className="primary-nav" aria-label={t('dashboard')}>{primaryNavigation}<div className="profile-nav"><button type="button" className={`main-nav-link profile-toggle${profileActive ? ' active' : ''}`} onClick={() => setProfileOpen((open) => !open)} aria-expanded={profileOpen}><CircleUserRound size={18} /><span>{t('profile')}</span></button>{profileOpen && <div className="profile-popover"><Link href="/profile" onClick={() => setProfileOpen(false)}><CircleUserRound size={17} />{t('profile')}</Link><Link href="/coach" onClick={() => setProfileOpen(false)}><BrainCircuit size={17} />ИИ-тренер</Link><Link href="/rides" onClick={() => setProfileOpen(false)}><Bike size={17} />{t('myRides')}</Link><Link href="/bikes" onClick={() => setProfileOpen(false)}><Wrench size={17} />{t('bikesAndService')}</Link><Link href="/settings" onClick={() => setProfileOpen(false)}><Settings size={17} />{t('settings')}</Link><button type="button" onClick={() => void signOut()}><LogOut size={17} />{t('signOut')}</button></div>}</div></nav><Link href="/coach" className={`header-icon-link header-coach-link${location.startsWith('/coach') ? ' active' : ''}`} aria-label="ИИ-тренер"><BrainCircuit size={20} /></Link><Link href="/messages" className={`header-icon-link${location.startsWith('/messages') ? ' active' : ''}`} aria-label={t('messages')}><MessageCircle size={20} /></Link><span className="header-preferences"><select aria-label={t('settings')} value={locale} onChange={(event) => changeLocale(event.target.value as Locale)}><option value="ru">RU</option><option value="kz">KZ</option><option value="en">EN</option></select><button type="button" aria-label={theme === 'light' ? 'Включить тёмную тему' : 'Включить светлую тему'} onClick={toggleTheme}>{theme === 'light' ? <Moon size={16} /> : <Sun size={16} />}</button></span></div> : <nav className="public-nav"><Link className="header-cta" href="/auth/sign-in">{t('signIn')}</Link><span className="header-preferences"><select aria-label={t('settings')} value={locale} onChange={(event) => changeLocale(event.target.value as Locale)}><option value="ru">RU</option><option value="kz">KZ</option><option value="en">EN</option></select><button type="button" aria-label={theme === 'light' ? 'Включить тёмную тему' : 'Включить светлую тему'} onClick={toggleTheme}>{theme === 'light' ? <Moon size={16} /> : <Sun size={16} />}</button></span></nav>}
    </header>
    {children}
    {session && <nav className="mobile-main-nav" aria-label={t('dashboard')}>{primaryNavigation}<MainLink href="/profile" label={t('profile')} icon={<CircleUserRound size={20} />} active={profileActive} /></nav>}
  </div>;
}
