  ...
  return <section className="pro-analyzer" aria-labelledby="pro-analyzer-title">
    <header className="pro-analyzer-heading">
      <div>
        ...
        <h2 id="pro-analyzer-title">{text('Собери свой идеальный сетап', 'Өзіңе мінсіз жинақ жаса', 'Build your ideal setup')}</h2>
        <p>{text(
          'Заполни параметры — ИИ разберёт велосипед, посадку, тренировки, маршруты, обслуживание и питание в одном отчёте.',
          'Параметрлерді толтыр — AI велосипедті, отырысты, жаттығуды, бағытты, қызмет көрсетуді және тамақтануды бір есепте талдайды.',
          'Add your details and AI will cover bike choice, fit, training, routes, maintenance and nutrition in one report.',
        )}</p>
      </div>
    ...
    {!active ? <div className={`pro-analyzer-locked${statusLoading ? ' loading' : ''}`}>
      ...
      <form className="pro-analysis-form" onSubmit={(event) => void analyze(event)}>
        <fieldset>
          <legend><span>01</span>{text('Параметры тела', 'Дене параметрлері', 'Measurements')}</legend>
        ...
        <fieldset>
          <legend><span>02</span>{text('Стиль и цель', 'Стиль және мақсат', 'Style and goal')}</legend>
        ...
        <fieldset>
          <legend><span>03</span>{text('Практические детали', 'Практикалық мәліметтер', 'Practical details')}</legend>
          <div className="pro-form-grid two">
