...
import type { InterviewAnswer, InterviewFeedback, InterviewSession } from './models';
...
export async function startInterview(essayId: string, content: string): Promise<InterviewSession> {
  ...
  if (!Array.isArray(session.questions) || !session.questions.every((item) => typeof item === 'string')) {
    throw new Error('Не удалось сохранить вопросы для интервью.');
  }
  return { id: session.id, questions: session.questions };
}
